...
void capturer_graines(Awale *jeu, int derniere_case) {
    int est_chez_adversaire = jeu->joueurCourant == 1 ? derniere_case >= 6 : derniere_case < 6;
    ...
    while(est_chez_adversaire) {
        if(jeu->plateau[derniere_case] == 2 || jeu->plateau[derniere_case] == 3) {
            ...
            est_chez_adversaire = jeu->joueurCourant == 1 ? derniere_case >= 6 : derniere_case < 6;
        } else {
            break;
        }
    }
...
int jouer_coup(Awale *jeu, int trou) {
    if (!coup_valide(jeu, trou)) return 0;
...
char* serialiser_jeu(Awale *jeu) {
    ...
    // Format: "GAMESTATE p0 p1 ... p11 scoreJ1 scoreJ2 joueurCourant fini gagnant"
    //ptr += sprintf(ptr, "GAMESTATE");
    for(int i = 0; i < 12; i++) {
        ptr += sprintf(ptr, " %d", jeu->plateau[i]);
    }
    sprintf(ptr, " %d %d %d %d %d %s %s", 
            jeu->scoreJ1, jeu->scoreJ2, 
            jeu->joueurCourant, jeu->fini, jeu->gagnant, jeu->pseudo1, jeu->pseudo2);
...
void deserialiser_jeu(Awale *jeu, const char *buffer) {
    if(strncmp(buffer, "GAMESTATE", 9) != 0) return;
